//! Integration smoke tests — readable end-to-end demos of the public API
//! (this target's `required-features` covers the full set).
//!
//! Capacity/bound semantics (duplicates consume no capacity, rejected elements
//! handed back, combined column caps, …) are covered exhaustively by the
//! differential harness in `tests/properties.rs`; don't add example tests for
//! those here. This file keeps the surfaces the harness doesn't drive:
//! borrowed-key lookups, store introspection, Hash/Ord citizenship, and one
//! readable demo per collection flavor.
...
fn arrayvec_fixed_capacity() {
    ...
    assert_eq!(s.try_insert(2).unwrap_err().into_inner(), 2);
